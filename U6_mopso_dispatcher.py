...
class MOPSOPlanner:
    ...
    def _evaluate(self, assignment: Dict[int, List[int]],
        ...
        for drone_id, order_ids in assignment.items():
            ...
            # Estimate energy using same model as environment
            # E = base_energy_per_distance * distance * (1 + alpha * load / max_capacity) * weather_battery_factor
            # Note: This is a simplified approximation
            base_energy = constraints.get('base_energy_per_distance', 0.1)
            energy_alpha = constraints.get('energy_alpha', 0.5)
            max_capacity = constraints.get('max_capacity_per_drone', drone.get('max_capacity', 10))
            weather_battery_factor = constraints.get('weather_battery_factor', 1.0)

            # Estimate average load during route (starts at current_load, increases with pickups, decreases with deliveries)
            # Simplified approximation: use current_load + half of new orders as average load
            # Assumes each order has weight 1.0 (not accounting for varying order weights)
            current_load = drone.get('current_load', 0)
            avg_load = current_load + len(order_ids) / 2.0
            # Ensure max_capacity is at least 1.0 to prevent division issues
            load_factor = 1.0 + energy_alpha * (avg_load / max(1.0, max_capacity))

            route_energy = base_energy * route_distance * load_factor * weather_battery_factor
